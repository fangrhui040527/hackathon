...
def _build_result_from_responses(
    ...
    # Extract product info from food data (case-insensitive)
    ...
    # Combine brand + title for a better product name
    brand = fd_lower.get("brand") or fd_lower.get("manufacturer") or fd_lower.get("contactinfo") or None
    if brand:
        from grounding_pipeline import _extract_brand_name
        brand = _extract_brand_name(str(brand))
    if brand and product_name:
        brand_str = str(brand).strip()
        if brand_str.lower() not in str(product_name).lower():
            product_name = f"{brand_str} {product_name}"
    ...
    nutrition_pills = []
    # First try parsed nutrition from grounding pipeline's raw_data
    raw_nutrition = None
    if enriched_data:
        raw_nutrition = enriched_data.get("raw_data", {})
    # Case-insensitive lookup across food_data keys
    food_data_lower = {k.lower(): v for k, v in food_data.items()} if isinstance(food_data, dict) else {}
    if not raw_nutrition or not isinstance(raw_nutrition, dict):
        raw_nutrition = food_data_lower
    else:
        raw_nutrition_lower = {k.lower(): v for k, v in raw_nutrition.items()} if isinstance(raw_nutrition, dict) else {}
        raw_nutrition = raw_nutrition_lower

    # Try to get a structured nutrition dict
    nutr = raw_nutrition.get("nutrition") or raw_nutrition.get("nutrition_facts") or raw_nutrition.get("nutrients") or {}
    # If nutrition is a string (from CU), parse it
    if isinstance(nutr, str):
        from grounding_pipeline import _parse_nutrition_string
        nutr = _parse_nutrition_string(nutr)
